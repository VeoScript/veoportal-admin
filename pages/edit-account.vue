...
let name = ref<string>('');
let opentowork = ref<boolean>(true);
...
watchEffect(() => {
  ...
  opentowork.value = user?.value?.isOpenToWork as boolean;
...
<template>
  <NuxtLayout>
    <div v-if="!isPendingUser" class="flex-1 w-full overflow-y-auto">
      ...
      <LoaderSubmit v-if="isLoading" message="Updating..." />
      <form v-on:submit="handleCreateProject" class="flex flex-col w-full p-5 md:p-10 space-y-5">
        <div class="flex flex-col items-center w-full">
          <div class="relative overflow-hidden w-[30vh] h-[30vh]">
            <NuxtImg preload class="w-full h-full object-cover rounded-full bg-accent-4"
              :src="`${imageSource === '' ? '/images/placeholder.png' : imageSource}`" alt="Project Image" />
            ...
            <input type="file" id="upload-image" class="hidden" accept=".jpg, .png, .jpeg, .jfif .webp"
              v-on:change="handleChangeImage" />
          </div>
        </div>
        <div class="flex flex-row items-center w-full space-x-2">
          <div class="flex flex-col w-full space-y-2">
            <label for="name" class="ml-3 font-light text-sm text-neutral-400">Name <span
                class="text-red-500">*</span></label>
            <input id="name" type="text"
              class="w-full p-3 outline-none rounded-xl border border-accent-4 bg-transparent focus:border-accent-1"
              v-model="name" v-on:input="() => nameError = ''">
            <span v-if="nameError" class="ml-3 font-light text-xs text-red-500">{{ nameError }}</span>
          </div>
          <div class="flex flex-col w-full space-y-2">
            <label for="opentowork" class="ml-3 font-light text-sm text-neutral-400">Open to work? <span
                class="text-red-500">*</span></label>
            <select id="opentowork"
              class="w-full p-3 outline-none rounded-xl border border-accent-4 bg-accent-3 focus:border-accent-1"
              v-model="opentowork">
        ...
        <div class="flex flex-row items-center w-full space-x-2">
          ...
          <div class="flex flex-col w-full space-y-2">
            <label for="email" class="ml-3 font-light text-sm text-neutral-400">Email <span
                class="text-red-500">*</span></label>
            <input disabled id="email" type="email"
              ...
              v-model="email" v-on:input="() => emailError = ''">
      ...
      <form v-on:submit="handleChangePassword" class="flex flex-col w-full p-10 space-y-10 border-t border-neutral-700">
        ...
        <div class="flex flex-col items-center w-full space-y-2">
          <div class="flex flex-col w-full space-y-2">
            <label for="oldpassword" class="ml-3 font-light text-sm text-neutral-400">Old Password <span
                class="text-red-500">*</span></label>
            <input id="oldpassword" type="password"
              class="w-full p-3 outline-none rounded-xl border border-accent-4 bg-transparent focus:border-accent-1"
              v-model="oldpassword" v-on:input="() => oldPasswordError = ''">
            <span v-if="oldPasswordError" class="ml-3 font-light text-xs text-red-500">{{ oldPasswordError }}</span>
          </div>
          <div class="flex flex-col w-full space-y-2">
            <label for="newpassword" class="ml-3 font-light text-sm text-neutral-400">New Password <span
                class="text-red-500">*</span></label>
            <input id="newpassword" type="password"
              class="w-full p-3 outline-none rounded-xl border border-accent-4 bg-transparent focus:border-accent-1"
              v-model="newpassword" v-on:input="() => newPasswordError = ''">
            <span v-if="newPasswordError" class="ml-3 font-light text-xs text-red-500">{{ newPasswordError }}</span>
          </div>
          <div class="flex flex-col w-full space-y-2">
            <label for="repassword" class="ml-3 font-light text-sm text-neutral-400">Re-enter Password <span
                class="text-red-500">*</span></label>
            <input id="repassword" type="password"
              class="w-full p-3 outline-none rounded-xl border border-accent-4 bg-transparent focus:border-accent-1"
              v-model="repassword" v-on:input="() => repasswordError = ''">
