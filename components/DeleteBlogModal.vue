...
const handleDeleteProject = async () => {
  ...
  await useFetch(`/api/blogs/delete/${props.id}`, {
    method: 'DELETE'
  })
...
<template>
  <div v-if="isOpen" class="absolute inset-0 z-10 flex flex-col items-center justify-center w-full h-full backdrop-blur-xl bg-black bg-opacity-50">
    <div class="z-20 flex flex-col items-center w-full max-w-xl p-10 space-y-10 rounded-xl border border-red-500 bg-accent-3">
      <p>Are you sure you want to delete this blog <span class="font-bold">{{ title }}</span>?</p>
