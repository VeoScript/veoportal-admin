...
<template>
  <button v-if="sideBarStore.isShow" @click="sideBarStore.setIsShow(false)"
    class="absolute z-20 flex md:hidden w-full h-full bg-white bg-opacity-50" />
  <div :class="sideBarStore.isShow ? 'flex' : 'hidden md:flex'"
    ...
    <NuxtLink to="/" @click="sideBarStore.setIsShow(false)">
      <NuxtImg preload class="w-12" src="/images/veo.png" alt="Veoscript Logo" />
    </NuxtLink>
    <div class="flex flex-col items-center w-full space-y-10">
      <div class="flex flex-col items-center w-full space-y-3">
        <div v-if="!user" class="w-auto">
          <NuxtImg preload class="w-20 h-20 rounded-full object-cover" src="/images/placeholder.png" alt="Veoscript" />
        </div>
        <div v-else class="w-auto">
          <NuxtImg preload class="w-20 h-20 rounded-full object-cover"
            :src="user?.image === '' ? '/images/placeholder.png' : String(user?.image)" alt="Veoscript" />
        </div>
        <h1 v-if="!user" class="font-bold text-sm">Loading...</h1>
        <h1 v-if="user" class="font-bold text-sm">{{ user?.name }}</h1>
      ...
      <div class="flex flex-col items-center w-full space-y-3">
        <div v-for="link in links" :key="link.name" class="w-full">
          <NuxtLink :to="link.link"
            ...
            @click="sideBarStore.setIsShow(false)">
            <svg v-if="link.name === 'Dashboard'" class="w-5 h-5 text-accent-2" fill="none" stroke="currentColor"
            ...
            <svg v-if="link.name === 'Projects'" class="w-5 h-5 text-accent-2" fill="none" stroke="currentColor"
              stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M3.75 9.776c.112-.017.227-.026.344-.026h15.812c.117 0 .232.009.344.026m-16.5 0a2.25 2.25 0 00-1.883 2.542l.857 6a2.25 2.25 0 002.227 1.932H19.05a2.25 2.25 0 002.227-1.932l.857-6a2.25 2.25 0 00-1.883-2.542m-16.5 0V6A2.25 2.25 0 016 3.75h3.879a1.5 1.5 0 011.06.44l2.122 2.12a1.5 1.5 0 001.06.44H18A2.25 2.25 0 0120.25 9v.776">
              </path>
            </svg>
            <svg v-if="link.name === 'Blogs'" class="w-5 h-5 text-accent-2" fill="none" stroke="currentColor"
              stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M12 7.5h1.5m-1.5 3h1.5m-7.5 3h7.5m-7.5 3h7.5m3-9h3.375c.621 0 1.125.504 1.125 1.125V18a2.25 2.25 0 01-2.25 2.25M16.5 7.5V18a2.25 2.25 0 002.25 2.25M16.5 7.5V4.875c0-.621-.504-1.125-1.125-1.125H4.125C3.504 3.75 3 4.254 3 4.875V18a2.25 2.25 0 002.25 2.25h13.5M6 7.5h3v3H6v-3z">
              </path>
            </svg>
            <svg v-if="link.name === 'Admin Users'" class="w-5 h-5 text-accent-2" fill="none" stroke="currentColor"
              stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round"
